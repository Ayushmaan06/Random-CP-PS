...
int main() {
    ...
    myFund.addEntity(make_unique<Asset>("Tech Stock A", 10000.0));
    myFund.addEntity(make_unique<Asset>("Tech Stock B", 15000.0));
    myFund.addEntity(make_unique<Liability>("Loan for Tech Expansion", 5000.0));


    myFund.showPortfolio();


    cout << "Total Value of Fund: $" << myFund.getTotalValue() << "\n";
    ...
    string searchName = "Tech Stock B";
    FinancialEntity* entity = myFund.findEntityByName(searchName);
    if (entity) {
        cout << "\nFound entity:\n";
        entity->showDetails();
    } else {
        cout << "\nEntity \"" << searchName << "\" not found in portfolio.\n";
    }
